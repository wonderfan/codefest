...
敏捷软件开发方法中就到处体现出了这一原则，并因此让软件开发变得精益而高效。平衡感、和谐感是建筑设计中的一个美学原则。

通常采用的功能模块结构如下图所示

微服务中，每个服务都是一个组件，通过编排组合的方式来使用，从而真正做到了独立、解耦、组件化、易维护、可复用、可替换、高可用、最终达到提高交付质量、缩短交付周期的效果。

如何高效率地开发一个庞大系统，同时做到后期维护成本低

单一职责、微、面向服务、自治、易扩展、流程化，技术特点：组件化、快速、可复用、机动灵活，组件的独立、可复用性、可替换性；微服务是组件化最彻底的架构，每一个服务是高内聚的组件，业务功能集中对外则是低耦合，发挥了小、快、灵的特性。

使用HTTP的RESTful API或轻量级的消息发送协议，实现信息传递与服务调用的触发。容错设计，演进式设计，微服务的理念以及特性，

鸟随鸾凤飞腾远，人伴贤良品自高

架构设计与功能要素 选型要素；API网关的核心设计理念是使用一个轻量级的消息网关作为所有客户端的应用入口，并且在 API 网关层面上实现通用的非功能性需求；

所有的服务通过 API 网关来暴露，这是所有客户端访问的唯一入口；如果一个服务要访问另一个服务，也要通过这个网关。
