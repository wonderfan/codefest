...
方法：计算计算资源的消耗，存储空间的消耗和网络资源的消耗。创新点：绿色共识机制；通过过滤机制来过滤掉，不需要的、无关的区块，从而减少本地存储的消耗；在交易和区块的验证的时候，在不同节点之间的消息广播存在重复、重叠，通过设计一种新的区块数据结构来避免消息传播的消耗。

原文地址：https://cse.buffalo.edu/~wenyaoxu/papers/journal/xu-tii2019.pdf

***

使用密码学和部分同步来实现二分拜占庭共识

为了实现拜占庭共识的活性，使用了密码学的证明和弱轮次协调员和部分同步的机制。

原文地址：https://arxiv.org/pdf/2001.07867.pdf

***

在新型应用中使用差分隐私和密码学机制

原文地址：https://arxiv.org/pdf/2004.08887.pdf

***

区块链分布式共识协议调查研究

使用区块提案、区块验证、消息传播、区块最终性、激励机制五个要素构成分析框架，来分析和比较去区块链的共识协议。

原文地址：https://arxiv.org/pdf/1904.04098.pdf
